...
def decision(update: Update, context: CallbackContext):
    ...
    if action == Txt.EQUIPPED_APPROVE_BUTTON_KEY:
        ...
        link_user = f'<a href="tg://user?id={user_id}">{fullname}</a>'
        ...
        update.effective_chat.send_message(Txt.APPROVED_HEADER.format(deal_id, link_user),
                                           parse_mode=ParseMode.HTML, timeout=30)
...
def comment(update: Update, context):
    ...
    if deal:
        ...
        link_user = f'<a href="tg://user?id={update.message.from_user.id}">{fullname}</a>'
        text_unapproved = Txt.DECLINED_HEADER.format(deal_id) + Txt.DEAL_DECLINE.format(link_user, text) + \
                          deal_message.text_html
        ...
        unapproved_chat.send_message(text_unapproved, reply_markup=InlineKeyboardMarkup(keyboard),
                                     parse_mode=ParseMode.HTML, timeout=30)
        deal_message.edit_text(text_unapproved, parse_mode=ParseMode.HTML, timeout=30)
        update.effective_chat.send_message(Txt.DECLINED_HEADER.format(deal_id), parse_mode=ParseMode.HTML,
                                           timeout=30)
    return ConversationHandler.END


def reapprove(update: Update, context: CallbackContext):
    orig_text = update.callback_query.message.text_html.split(Txt.DELIMITER_BLOCK_TEXT)[1]
    ...
    context.bot.send_message(chat_id=creds.EQUIPPED_GROUP_CHAT_ID, text=orig_text,
                             parse_mode=ParseMode.HTML, reply_markup=keyboard, timeout=30)

    text = update.callback_query.message.text_html.replace(Txt.DECLINED_HEADER.format(deal_id),
                                                                  Txt.REAPPROVED_HEADER.format(deal_id))
    update.callback_query.edit_message_text(text, parse_mode=ParseMode.HTML, timeout=30)
