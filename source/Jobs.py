...
def deal_equipped(context: CallbackContext):
    ...
    with BW.OAUTH_LOCK:
        ...
        deal_id = Utils.prepare_external_field(query_components, WEBHOOK_DEAL_ID_ALIAS, escape_md=False)
        deal_responsible = Utils.prepare_external_field(query_components, WEBHOOK_DEAL_RESPONSIBLE_ALIAS, escape_md=False)
        deal_florist = Utils.prepare_external_field(query_components, WEBHOOK_DEAL_FLORIST_ALIAS, escape_md=False)
        deal_order = Utils.prepare_external_field(query_components, WEBHOOK_DEAL_ORDER_ALIAS, escape_md=False)
        deal_courier = Utils.prepare_external_field(query_components, WEBHOOK_DEAL_COURIER_ALIAS, escape_md=False)
        deal_accepted = Utils.prepare_external_field(query_components, WEBHOOK_ACCEPTED_ALIAS, escape_md=False)
        deal_sum = Utils.prepare_external_field(query_components, WEBHOOK_SUM_ALIAS, escape_md=False)
        deal_date = Utils.prepare_external_field(query_components, WEBHOOK_DATE_ALIAS, escape_md=False)
        deal_time = Utils.prepare_external_field(query_components, WEBHOOK_TIME_ALIAS, escape_md=False)
        deal_type = Utils.prepare_external_field(query_components, WEBHOOK_TYPE_ALIAS, escape_md=False)
        ...
        if deal_id != Txt.FIELD_IS_EMPTY_PLACEHOLDER:
            ...
            if photo_urls:
                ...
                deal_message = '<b>Фото не найдены!</b>\n\n' + deal_message

            bot.send_message(chat_id=creds.EQUIPPED_GROUP_CHAT_ID,
                             text=deal_message,
